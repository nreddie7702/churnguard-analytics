"""
Builds and validates sql/business_questions.sql — 25 interview-level SQL
business questions against churnguard.db. Every query is executed here
to confirm it runs without error before being written to the .sql file,
and a handful of key results are printed so you can sanity-check the
business story they tell.
"""
...
OUT_PATH = "sql/business_questions.sql"

conn = sqlite3.connect(DB_PATH)

# Each entry: (number, business_question, sql, techniques_demonstrated)
QUERIES = [

(1, "What is our current total MRR (active subscriptions only)?",
"""SELECT ROUND(SUM(mrr_value), 2) AS current_mrr
FROM subscriptions
WHERE status = 'active';""",
"Aggregation, WHERE"),

(2, "How does current MRR break down by plan tier?",
"""SELECT plan_tier,
...
GROUP BY plan_tier
ORDER BY mrr DESC;""",
"GROUP BY, window function (SUM OVER)"),

(3, "What has new MRR looked like month by month for the last 12 months?",
...
"Date functions, GROUP BY, subquery"),

(4, "What has churned MRR looked like month by month?",
...
"Date functions, GROUP BY, CASE-free filtering"),

(5, "What is the net new MRR movement (new minus churned) by month?",
"""WITH new_mrr AS (
...
(6, "What is our monthly logo churn rate (% of active customers who canceled)?",
...
"CTE, LEFT JOIN, NULLIF"),

(7, "Which industries have the highest revenue churn rate?",
"""SELECT c.industry,
...
"JOIN, CASE WHEN, HAVING, aggregation"),

(8, "Rank customers by current MRR within their industry.",
...
LIMIT 30;""",
"Window function RANK, PARTITION BY, JOIN"),

(9, "Who are our top 10 customers by total lifetime revenue actually paid?",
...
LIMIT 10;""",
"Multi-table JOIN, GROUP BY, aggregation"),

(10, "Dense-rank plan tiers by their churn rate (ties get the same rank).",
"""WITH tier_churn AS (
    SELECT plan_tier,
...
(11, "Build a signup-month cohort retention table: what % of each cohort is still active N months later?",
"""WITH cohorts AS (
...
"CTE, JOIN, date math (julianday), CASE WHEN"),

(12, "What is the average tenure (in days) before churn, by plan tier?",
"""SELECT plan_tier,
...
(13, "For each customer, show login_count this month vs. last month (to spot engagement drops).",
...
LIMIT 30;""",
"Window function LAG, PARTITION BY"),

(14, "Which active customers had a >50% drop in logins month-over-month (early churn warning)?",
...
ORDER BY pct_change
LIMIT 20;""",
"CTE, LAG, multi-JOIN, at-risk filtering"),

(15, "What is our monthly payment failure rate?",
...
GROUP BY month
ORDER BY month;""",
"Date functions, CASE WHEN, aggregation"),

(16, "Which currently-active customers have had 2+ failed payments in their last 3 invoices (at-risk list)?",
...
"ROW_NUMBER, CTE, HAVING"),

(17, "Which support ticket categories generate the highest volume, and what's their average resolution time?",
"""SELECT category,
...
"GROUP BY, HAVING, date math"),

(18, "Do churned customers have lower average CSAT scores than retained customers?",
"""SELECT s.status,
...
GROUP BY s.status;""",
"JOIN, GROUP BY, filtering NULLs"),

(19, "Show each customer's most recent support ticket only.",
...
(20, "How many days has it been since each active customer last logged in (engagement recency)?",
...
LIMIT 20;""",
"Subquery, JOIN, date math, GROUP BY"),

(21, "Compare acquisition channels on CAC vs. average revenue generated per customer.",
...
"Multi-JOIN, aggregation, ratio calculation"),

(22, "What's the running total of new MRR added, month over month, across the whole timeline?",
"""WITH monthly_new AS (
...
FROM monthly_new
ORDER BY month;""",
"CTE, window function SUM OVER with ORDER BY (running total)"),

(23, "Find customers who upgraded and then later downgraded (a red flag pattern).",
...
"LAG, pattern detection across rows"),

(24, "What % of total company MRR comes from our top 10% of customers (revenue concentration risk)?",
"""WITH ranked AS (
...
FROM ranked;""",
"NTILE, CASE WHEN, concentration analysis"),

(25, "Give a full customer risk snapshot: plan, MRR, days since last login, recent payment failures, open tickets.",
...
LIMIT 20;""",
"Multi-table LEFT JOIN, GROUP BY, HAVING — composite risk view"),
]

# ------------------------------------------------------------------
# TEST every query, then write the .sql file
# ------------------------------------------------------------------
sql_file_lines = [
...
failures = []
for num, question, query, techniques in QUERIES:
    try:
        df = pd.read_sql(query, conn)
        status = f"OK ({len(df)} rows)"
    except Exception as e:
        status = f"FAILED: {e}"
        failures.append((num, question, e))

    print(f"Q{num:02d} [{status}] {question}")

    sql_file_lines.append(f"-- Q{num}: {question}")
    sql_file_lines.append(f"-- Techniques: {techniques}")
...
print(f"\n{'='*55}")
if failures:
    print(f"{len(failures)} QUERIES FAILED — see above")
else:
    print(f"ALL 25 QUERIES PASSED — saved to {OUT_PATH}")
print(f"{'='*55}")
